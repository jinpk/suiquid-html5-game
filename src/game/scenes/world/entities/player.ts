...
export class Player extends Sprite implements IPlayer {
  ...
  protected speed: number = DIFFICULTY.PLAYER_SPEED;

  private movementKeys: Nullable<Record<string, Phaser.Input.Keyboard.Key>> =
    null;
  ...
  constructor(scene: IWorld, data: PlayerData) {
    ...
    this.setTilesCollision(
      [TileType.MAP, TileType.BUILDING, TileType.CHEST],
      (tile) => {
    ...
    this.scene.physics.add.collider(
      this,
      this.scene.entityGroups.enemies,
      (_, enemy: IEnemy) => {
        enemy.attack(this);
      }
    );
  ...
  private addAssistant() {
    const positionAtMatrix = aroundPosition(this.positionAtMatrix, 1).find(
      (spawn) => {
  ...
  private registerKeyboard() {
    this.movementKeys = <Record<string, Phaser.Input.Keyboard.Key>>(
      this.scene.input.keyboard.addKeys(CONTROL_KEY.MOVEMENT)
    );
  }
  ...
  protected getKeyboardSingleDirection(
  ...
  private addVisitedWay() {
    ...
    if (
      [BiomeType.SAND, BiomeType.GRASS].includes(
        this.currentGroundTile.biome.type
      )
    ) {
