...
export class Sprite extends Phaser.Physics.Arcade.Sprite implements ISprite {
  ...
  private collisionPlayer: OPlayer;
  private collisionPlayerHandler: Nullable<
    (player: Phaser.GameObjects.Image) => void
  > = null;
  ...
  constructor(
    scene: IWorld,
    { texture, positionAtMatrix, health, frame = 0 }: SpriteData
  ) {
  ...
  public update() {
    ...
    this.currentGroundTile = this.scene.level.getTile({
  ...
  public setObjectCollision(
    target: OPlayer,
    handler: (object: Phaser.GameObjects.Image) => void
  ) {
    this.collisionPlayer = target;
    this.collisionPlayerHandler = handler;
  }
  ...
  private getCollidedTile(direction: number) {
    ...
    const target = this.scene.physics.velocityFromAngle(
      direction,
      WORLD_COLLIDE_LOOK
    );
    const occupiedTiles = this.getCorners().map((point) =>
      Level.ToMatrixPosition({
        x: point.x + target.x,
        y: point.y + target.y,
      })
    );
  ...
  private updateHealthIndicator() {
    ...
    bar.setSize(
      (this.healthIndicator.width - 2) * value,
      this.healthIndicator.height - 2
    );
  }
