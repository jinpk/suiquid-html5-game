...
import { OPlayer } from "~entity/oplayer";
...
export class World extends Phaser.Scene implements IWorld {
  ...
  private set entityGroups(v) {
    this._entityGroups = v;
  }
  ...
  private set particles(v) {
    this._particles = v;
  }

  private _players: IOPlayer[] = [];

  public get players() {
    return this._players;
  }

  private set players(v) {
    this._players = v;
  }
  ...
  public spawnEnemy(variant: EnemyVariant): IEnemy {
    const buildings = this.getBuildings();
    const allowedPositions = this.enemySpawnPositions.filter(
      (position) =>
    ...
    const positions = sortByDistance(
      allowedPositions,
      this.player.positionAtMatrix
